...
import { GoalsResult } from "../types/goal.types";

export const goalApi = createApi({
  reducerPath: "goals",
  baseQuery: axiosBaseQuery({ baseUrl: "/goals" }),
  endpoints: (builder) => ({
    getGoals: builder.query<GoalsResult, string>({
      query: (id: string) => ({ url: `/${id}`, method: "GET" }),
    }),
    createGoal: builder.mutation<ResponseWithoutData, CreateGoalDto>({
      query: (data: CreateGoalDto) => ({
        url: "/",
        method: "POST",
        data,
      }),

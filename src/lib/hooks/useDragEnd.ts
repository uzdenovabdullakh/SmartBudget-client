...
export const useDragEnd = (
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (sourceGroup.id === targetGroup.id) {
      ...
      const newIndex = targetGroup.categories.findIndex((c) => c.id === overId);
    ...
    const movedCategory = sourceGroup.categories.find((c) => c.id === activeId);
    ...
    const newTargetCategories = [...targetGroup.categories, movedCategory].map(
